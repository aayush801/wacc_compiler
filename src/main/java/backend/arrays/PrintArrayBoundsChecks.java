...
public class PrintArrayBoundsChecks {
    ...
    public static PrimitiveLabel printArrayIndexCheck(ProgramGenerator program) {
        ...
        instructions.add(new Load(ConditionCode.LT, new Register(0), new Address(negLabel.getLabelName()), false, false));
        ...
        instructions.add(new Load(ConditionCode.CS, new Register(0), new Address(largeLabel.getLabelName()), false, false));

        // BLCS p_throw_runtime_error
        instructions.add(new Branch(ConditionCode.CS, "p_throw_runtime_error", true));


        return new PrimitiveLabel("check_array_bounds", program.encapsulateFunction(instructions));
    }
